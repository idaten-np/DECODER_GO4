...
//printf("%x l_tdc_trail\n",*pl_tmp);
	...
	/*if (!fCalibrationDone && ((l_phy_evt_ct >= N_CAL_EVT) || fPar->useOldCalibration))
	{
		...
		for (iCHA=0; iCHA<MAX_CHA; iCHA++)
		{
			...
			{
				sum += h_box[iSSY][iSFP][iTAM][iCHA]->GetBinContent(l_i+1);
	...
	if(fCalibrationDone)
	{
		...
		for (l_i=0; l_i<MAX_CHA_AN; l_i+=2) // l_i: analyzer index
		{
